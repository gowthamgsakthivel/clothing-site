...
const AssetControlsPage = () => {
    ...
    const loadFeaturedData = async () => {
      ...
      try {
        const [featuredResponse, productsResponse] = await Promise.all([
          ...
          fetch('/api/product/list?limit=100')
        ]);
  ...
  const visibleAvailableProducts = useMemo(() => {
    const searchTerm = featuredSearch.trim().toLowerCase();
  ...
  }, [availableProducts, featuredProductIds, featuredSearch]);
            ...
            <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
              <div className="flex flex-col gap-3 border-b border-slate-200 px-5 py-4 md:flex-row md:items-center md:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-500">Select which products should appear on the home page.</p>
                </div>
                <button
                  type="button"
                ...
                >
                  {featuredSaving ? 'Saving...' : 'Save featured products'}
                </button>
              </div>
                  ...
                  <>
                    <div>
                      <div className="mb-3 flex items-center justify-between gap-3">
                        ...
                        <span className="text-xs font-medium text-slate-500">{featuredProductIds.length} selected</span>
                      </div>

                      {featuredProducts.length ? (
                        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                          {featuredProducts.map((product) => (
                            <div key={product._id} className="rounded-2xl border border-slate-200 bg-slate-50 p-3 shadow-sm">
                              ...
                              <div className="mt-3 flex items-start justify-between gap-3">
                                <div className="min-w-0">
                                  ...
                                  <p className="mt-0.5 truncate text-xs text-slate-500">{product.brand || product.category || 'Product'}</p>
                                </div>
                                <button
                                  type="button"
                    ...
                    <div className="border-t border-slate-200 pt-5">
                      <div className="mb-3 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                        <h3 className="text-sm font-semibold text-slate-900">Available products</h3>
                        <input
                          value={featuredSearch}
                          onChange={(e) => setFeaturedSearch(e.target.value)}
                          placeholder="Search products"
                          className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm md:max-w-xs"
                        />
                      </div>
                      ...
                      {visibleAvailableProducts.length ? (
                        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                          {visibleAvailableProducts.map((product) => (
                            <div key={product._id} className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
                              ...
                              <div className="mt-3 flex items-start justify-between gap-3">
                                ...
                                <button
                                  type="button"
                                  onClick={() => toggleFeaturedProduct(product)}
                                  className="shrink-0 rounded-lg border border-orange-200 bg-orange-50 px-3 py-1.5 text-xs font-medium text-orange-700 hover:bg-orange-100"
                                >
                                  Add
                                </button>
                              </div>
          ...
          <div className="space-y-6 lg:sticky lg:top-6">
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <ul className="mt-3 space-y-2 text-sm text-slate-600">
                ...
                <li>• Featured products shown on the home page</li>
              </ul>
            </section>
          </div>
        </div>
